<template>
				...
				<div class="p-4 bg-accent">
					<div class="flex flex-wrap gap-2 mb-3">
						...
						<select v-model="statusFilter"
							class="px-3 w-36 py-2 border border-default rounded-lg bg-default">
					...
					<div class="overflow-x-auto">
						<table class="min-w-full text-sm border border-default border-collapse">
							<thead>
								<tr class="bg-default text-gray-300 border-b border-default">
							...
							<tbody>
								<tr v-if="!loading && filteredRows.length === 0">
									<td colspan="8" class="px-4 py-6 text-center text-gray-400 border border-default">
										No links found.
									</td>
								</tr>

								<tr v-for="it in filteredRows" :key="it.id"
									class="hover:bg-white/5 transition border border-default">
									<!-- Title -->
									<td class="px-4 py-2 border border-default">
										<div v-if="editingId !== it.id" class="flex items-center gap-2">
											...
											<button class="text-xs text-blue-300 hover:underline"
												@click="startEdit(it)">Edit</button>
										</div>
										<div v-else class="flex items-center gap-2">
											<input v-model="editTitle"
												class="px-2 py-1 rounded bg-default border border-default w-56" />
											<button class="px-2 py-1 rounded bg-[#5E56C5]"
									...
									<td class="px-4 py-2 border border-default">
										<div class="flex items-center gap-2">
											<a :href="it.shortUrl" target="_blank" rel="noopener"
												class="underline truncate max-w-[22rem]">
												{{ it.shortUrl }}
											</a>
											<button class="text-blue-300 hover:underline text-xs"
									...
									<td class="px-4 py-2 border border-default ">
										...
										<div v-if="!expEditor[it.id]?.open"
											class="button-group-row justify-between items-center gap-1 mt-1">
											...
											<div class="flex flex-row">
												<label class="flex items-center gap-1">
													...
													<input type="number" min="0"
														class="input-textlike h-7 w-16 text-left"
									...
									<td class="px-4 py-2 border border-default">
										<div class="flex flex-wrap gap-2">
											...
											<button class="btn btn-primary px-2 py-1 rounded-md"
												@click="copy(it.shortUrl)">Copy</button>
											<button class="btn btn-secondary px-2 py-1 rounded-md"
    ...
    <div v-if="showModal" class="fixed inset-0 z-40">
      ...
      <div class="absolute inset-x-0 top-12 mx-auto w-11/12 max-w-5xl
                    bg-[#24272e] border border-default rounded-lg shadow-lg z-50">
        <!-- Header -->
        <div class="flex items-center justify-between px-4 py-3 border-b border-default">
          <h3 class="text-lg font-semibold">
            Link Details — {{ current?.title || (current && fallbackTitle(current)) }}
          </h3>
          <button class="px-2 py-1 rounded border border-default" @click="closeModal">Close</button>
        </div>
        ...
        <div class="px-4 pt-4 grid grid-cols-1 lg:grid-cols-3 gap-4 text-sm">
          <section class="space-y-2">
            ...
            <div class="break-all">
              ...
              <button class="ml-2 text-blue-300 hover:underline text-xs"
                @click="copy(current?.shortUrl)">Copy</button>
            </div>
            <div><span class="text-gray-400">Created:</span> {{ current ? new
              Date(current.createdAt).toLocaleString() : '' }}</div>
            <div><span class="text-gray-400">Expires:</span> {{ current?.expiresAt ? new
              Date(current.expiresAt).toLocaleString() : 'Never' }}</div>
            ...
            <div class="pt-2">
              ...
              <input v-model="drawerTitle"
                class="w-full px-3 py-2 rounded bg-default border border-default" />
            </div>
            <div>
              <label class="block text-gray-300 mb-1">Notes</label>
              <textarea v-model="drawerNotes" rows="3"
                class="w-full px-3 py-2 rounded bg-default border border-default"></textarea>
            </div>
