<template>
    <div v-if="modelValue" class="fixed inset-0 z-40">
      ...
      <div class="absolute inset-x-0 top-12 mx-auto w-11/12 max-w-5xl bg-well border border-default rounded-lg shadow-lg z-50">
        <!-- Header -->
              ...
              <table class="min-w-full text-sm border-separate border-spacing-0">
                ...
                <tbody>
                  <tr v-for="f in files" :key="f.key" class="border-t border-default">
                    ...
                    <td v-if="link?.type === 'upload'" class="px-3 py-2 break-all border border-default">{{ f.saved_as || '—' }}</td>
                    <td class="px-3 py-2 text-right border border-default">{{ fmtBytes(f.size) }}</td>
                    <td class="px-3 py-2 border border-default">{{ f.mime || '—' }}</td>
                    <td v-if="link?.type === 'upload'" class="px-3 py-2 border border-default">{{ f.uploader_label || '—' }}</td>
              ...
              <table class="min-w-full text-sm border-separate border-spacing-0">
                ...
                <tbody>
                  <tr v-for="u in access" :key="u.user_id" class="border-t border-default">
                    <td class="px-3 py-2 border border-default break-all">{{ u.name || '—' }}</td>
                    ...
                    <td class="px-3 py-2 border border-default">
                      <span v-if="u.display_color" class="inline-block w-4 h-4 rounded border align-middle" :style="{ backgroundColor: u.display_color }" :title="u.display_color" />
...
import type { LinkItem, LinkType, AccessRow, Status } from '../../typings/electron'
  ...
  async function savePassword() {
    ...
    await props.apiFetch(`/api/links/${props.link.id}`, {
      method: 'PATCH',
      body: JSON.stringify({ password: drawerPassword.value })
    })
    drawerPassword.value = ''
    emit('updated', { id: props.link.id, passwordRequired: true })
  }
  ...
  async function clearPassword() {
    ...
    emit('updated', { id: props.link.id, passwordRequired: false })
  }
